...
class VaDEIV(torch.nn.Module):    
    ...
    def encode_zt(self,x):
        ...
        return self.mu_zt(h), self.log_var_zt(h).clamp(min=-10, max=10)
    ...
    #     return self.prior_mu_zt(h), self.prior_log_var_zt(h).clamp(min=-10, max=10)
    ...
    def forward(self, x,t,y):
        ...
        return x_hat, mu_zc, mu_zt, log_var_zc, log_var_zt, zc,zt, logit_t, y_sample, t_rep,t_sample
    ...
    def compute_loss(self,x,t,y):
        if self.use_dist_net and self.use_reg_net:
            x_hat, mu_zc, mu_zt, log_var_zc, log_var_zt, zc,zt, logit_t, y_sample, t_rep,t_sample = self.forward(x,t,y)        
        ...
        kl_zt = torch.sum(-0.5 * torch.sum(1 + log_var_zt - mu_zt ** 2 - log_var_zt.exp(), dim = 1), dim = 0)
